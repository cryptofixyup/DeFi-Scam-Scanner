...
async def active_plan(session: AsyncSession, user_id: UUID) -> Plan:
    ...
    row = await session.execute(
        ...
        .where(Subscription.user_id == user_id, Subscription.status == "active", Subscription.period_start <= now, Subscription.period_end > now, Plan.active.is_(True))
...
async def reserve_daily_scan(session: AsyncSession, user_id: UUID) -> Plan:
    ...
    await session.execute(text("SELECT pg_advisory_xact_lock(hashtext(:key))"), {"key": str(user_id)})
    ...
    stmt = insert(UsageCounter).values(user_id=user_id, usage_date=date.today(), scan_count=1)
...
async def release_scan(session: AsyncSession, user_id: UUID) -> None:
    await session.execute(
        ...
        .where(UsageCounter.user_id == user_id, UsageCounter.usage_date == date.today(), UsageCounter.scan_count > 0)
...
async def usage_month(session: AsyncSession, user_id: UUID) -> int:
    ...
    value = await session.scalar(select(func.coalesce(func.sum(UsageCounter.scan_count), 0)).where(UsageCounter.user_id == user_id, UsageCounter.usage_date >= first_day))
    return int(value or 0)
